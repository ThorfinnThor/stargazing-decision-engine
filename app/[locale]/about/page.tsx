...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const de = locale === "de";
  return { title: de ? "Über Stargazing Index" : "About Stargazing Index", description: de ? "Mission, Arbeitsweise und redaktionelle Verantwortung von Stargazing Index." : "The mission, working principles, and editorial responsibility behind Stargazing Index.", alternates: { canonical: localizedLinks.about(locale), languages: Object.fromEntries(locales.map((item) => [item, localizedLinks.about(item)])) } };
}
...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <LegalPageShell locale={locale} eyebrow={de ? "Über uns · Redaktion" : "About · editorial"} title={de ? "Entscheidungen für bessere Nächte." : "Decisions for better nights."} description={de ? "Stargazing Index verbindet reproduzierbare Astronomie- und Klimadaten mit klarer, unabhängiger Reise- und Ausrüstungsorientierung." : "Stargazing Index combines reproducible astronomy and climate data with clear, independent travel and gear guidance."}>
      ...
      <section className="event-summary" aria-labelledby="about-corrections-title"><h2 id="about-corrections-title">{de ? "Korrekturen und Hinweise" : "Corrections and feedback"}</h2><p>{de ? "Wenn du einen Datenfehler, eine veraltete Zugangsregel oder eine unklare Aussage findest, schreibe uns. Hinweise werden geprüft; bei einer Korrektur aktualisieren wir den betroffenen Inhalt und sein Prüfdatum." : "If you find a data error, outdated access rule, or unclear statement, tell us. We review reports and update both the affected content and its review date when a correction is made."}</p><a className="text-link" href={localizedLinks.contact(locale)}>{de ? "Kontakt aufnehmen →" : "Contact us →"}</a></section>
    </LegalPageShell>
  );
}
