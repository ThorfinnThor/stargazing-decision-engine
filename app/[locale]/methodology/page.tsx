...
export default async function MethodologyPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="event-page" lang={isGerman ? "de" : "en"}>
      ...
      <section className="event-summary" aria-labelledby="method-independence-title"><h2 id="method-independence-title">{isGerman ? "Redaktionelle Unabhängigkeit" : "Editorial independence"}</h2><p>{isGerman ? "Eine mögliche Provision verändert weder Vergleichskriterien noch Schlussfolgerungen. Kommerzielle Links werden als solche gekennzeichnet; Partner können keine Platzierung kaufen. Unsere Guides bleiben auch ohne Affiliate-Link vollständig lesbar." : "A potential commission does not change comparison criteria or conclusions. Commercial links are identified; partners cannot buy placement. Every guide remains complete without an affiliate link."}</p><p>{isGerman ? "Einzeln ausgewählte Aktivitäten werden eindeutig einer Destination zugeordnet. Ergänzende Viator- und GetYourGuide-Suchlinks übergeben den Namen der jeweiligen Destination sowie wahlweise den Begriff Stargazing. Auf allen Destination- und zugehörigen Tourseiten zeigen GetYourGuide-Widgets wechselnde Live-Ergebnisse für das jeweilige Ziel. Preise, Bewertungen und Verfügbarkeiten übernehmen wir nicht in den statischen Katalog, weil sie sich ändern können." : "Individually selected activities are explicitly matched to a destination. Additional Viator and GetYourGuide search links send the relevant destination name and, where selected, the term stargazing. Every destination and its location-tour page includes a GetYourGuide widget with changing live results for that place. Prices, ratings, and availability are not copied into the static catalog because they can change."}</p></section>
      <section className="event-summary" aria-labelledby="method-maintenance-title"><h2 id="method-maintenance-title">{isGerman ? "Aktualität, Korrekturen und Quellen" : "Freshness, corrections, and sources"}</h2><p>{isGerman ? "Jeder Guide verlinkt seine relevanten Originalquellen. Bei technischen oder sicherheitsrelevanten Aussagen hat die aktuelle Dokumentation des Herstellers oder Betreibers Vorrang. Fehler und veraltete Angaben werden korrigiert." : "Every guide links its relevant original sources. Current manufacturer or operator documentation takes priority for technical or safety-critical claims. Errors and outdated information are corrected."}</p><p>{isGerman ? "Für Bilder und externe Daten veröffentlichen wir nur Inhalte mit vollständigem Quellen- und Lizenznachweis aus freigegebenen Open-Access- oder Public-Domain-Lizenzen." : "For images and external data, we publish only material with complete source and licence records under approved open-access or public-domain terms."}</p><a className="text-link" href={localizedLinks.gear(locale)}>{isGerman ? "Zu allen Ausrüstungs-Guides →" : "Browse all gear guides →"}</a></section>
    </main>
  );
}
